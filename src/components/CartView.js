        ...
        cart.length > 0 ? (
          <>
                ...
                return(
                  <div key={item.id} className='w-[60%] flex relative h-[100%] bg-dimWhite m-1 bg-white text-black'>
                    <div className='flex justify-center items-center w-[12%]'>
                      <button onClick={() => handleDeleteProduct(item.id)} className='btn m-1 p-1 bg-black text-white'>Eliminar{/* <FaTrashAlt/> */}</button>
                    </div>
                    <div className='flex items-center justify-center w-[15%]'>
            ...
            <div className='w-[60%] flex justify-end m-10 gap-5'>
              ...
              <Link to='/' className='btn bg-white text-black'>Finalizar Compra</Link>
            </div>
          </> 
        ) : (
